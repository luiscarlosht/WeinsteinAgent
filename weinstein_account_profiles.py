...
def read_fidelity_positions(csv_path: str) -> pd.DataFrame:
    ...
    Fidelity positions files may contain a BOM, trailing commas, and footer rows.
    """
...
def normalize_positions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Remove cash pseudo symbols from recommendation matching, but keep them in totals.
    out["IsCash"] = out.get("Symbol", "").astype(str).str.contains("CASH|SPAXX|FCASH|MONEY MARKET", case=False, regex=True, na=False)
    return out
