...
EXPECTED_BOOL_COLS = [
    ...
    "sell_confirm",
    "buy_confirm",
    # NEW: Chapter 8 filter — from intraday watcher debug
    "cond_market_long_ok",
]
EXPECTED_NUM_COLS = [
    "price",
...
def main():
    ...
    for _, r in df.iterrows():
        ...
        # Chapter 8 flag from watcher: if false → BEAR, no new longs
        market_long_ok = bool(r.get("cond_market_long_ok", True))
        ...
        # If Chapter 8 says NO LONGS (BEAR), hard-reset BUY state to avoid stale triggers.
        if not market_long_ok:
            st["buy_state"] = "IDLE"
            st["near_hits"] = []
            st["cooldown"] = 0

        # --- BUY progression (only when market_long_ok)
        if market_long_ok:
        ...
        # --- SELL progression (always allowed; even in BEAR you still want exits)
