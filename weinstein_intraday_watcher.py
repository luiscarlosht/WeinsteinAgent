...
# ---- Optional Google Sheets pull (Signals + Mapping for crypto universe) ----
try:
    import gspread
...
CRYPTO_BENCHMARK  = "BTC-USD"  # for RS
...
NEAR_VOL_PACE_MIN = 1.00  # <-- restored
...
# ----- Weinstein weekly-like params for crypto summary -----
WEEKS_LOOKBACK = 180
MA_WEEKS = 30
MA10_WEEKS = 10
RS_MA_WEEKS = 30
SLOPE_WINDOW = 5
NEAR_MA_BAND = 0.05
TOP_N_TINY_CHARTS = 30  # in crypto table
...
def _colored_summary_html(m):
    ...
    <style>
      ...
      .tinytbl { border-collapse: collapse; width:100%; }
      .tinytbl th, .tinytbl td { padding: 6px 8px; border-bottom: 1px solid #eee; font-size: 13px; vertical-align: top; }
...
# ---- Tiny inline chart (data URI) for crypto table ----
...
def _make_tiny_chart_html_from_series(series_price: pd.Series, bench_series: pd.Series) -> str:
    ...
    return f'<img src="{img_src}" alt="chart" style="display:block;width:100%;max-width:220px;height:auto;border:0" />'
...
# ---------------- Google Sheets helpers (Signals → crypto universe) ----------------
...
def _signals_crypto_universe(sheet_url: str, service_account_file: str) -> list[str]:
    """Harvest crypto tickers from the existing 'Signals' tab (optionally map via 'Mapping')."""
...
# ---------------- Weekly-like engine for Crypto Summary ----------------
...
def compute_stage_for_ticker_weekly(closes: pd.Series, bench: pd.Series):
...
def _fetch_weekly(tickers, benchmark, weeks=WEEKS_LOOKBACK):
...
def _crypto_df_to_html(df: pd.DataFrame, title: str, summary_line: str):
    ...
    html = f"""
    <h3>{title}</h3>
    <div style="color:#666;margin:-6px 0 12px 0;">Generated {now}</div>
...
def _attach_tiny_charts_crypto(df: pd.DataFrame, close_w: pd.DataFrame, bench_series: pd.Series, top_n: int = TOP_N_TINY_CHARTS) -> pd.DataFrame:
    out = df.copy()
    out["chart"] = ""
    for i, row in out.head(top_n).iterrows():
        ...
        if t in close_w.columns:
            try:
                out.at[i, "chart"] = _make_tiny_chart_html_from_series(close_w[t], bench_series)
            except Exception:
                out.at[i, "chart"] = ""
    return out

def build_crypto_summary_block(sheet_url: str, service_account_file: str) -> str:
    """Builds the Crypto summary HTML block from Signals tab."""
    crypto_tickers = _signals_crypto_universe(sheet_url, service_account_file)
    crypto_tickers = [t for t in crypto_tickers if _is_crypto_symbol(t)]
    if not crypto_tickers:
        return ""  # nothing to show
    close_w, volume_w = _fetch_weekly(crypto_tickers + [CRYPTO_BENCHMARK], CRYPTO_BENCHMARK, weeks=WEEKS_LOOKBACK)
    if close_w is None or CRYPTO_BENCHMARK not in close_w.columns:
        return ""
    ...
    for t in crypto_tickers:
        if t not in close_w.columns:
            ...
            continue
        res = compute_stage_for_ticker_weekly(close_w[t], bench_series)
        res["ticker"] = t
    ...
    df["buy_signal"] = df["stage"].apply(lambda s: _classify_buy_signal(str(s)))
    ...
    with_charts = _attach_tiny_charts_crypto(df, close_w, bench_series, top_n=TOP_N_TINY_CHARTS)
...
def run(_config_path="./config.yaml"):
    ...
    # ---------- Crypto Summary from Signals (APPENDED AT END) ----------
    crypto_block = ""
    if sheet_url and service_account_file and os.path.exists(service_account_file):
        try:
            crypto_block = build_crypto_summary_block(sheet_url, service_account_file)
        except Exception as e:
            log(f"Crypto summary build failed: {e}", level="warn")

    if crypto_block:
        html += "<hr/>" + crypto_block  # <-- at the end of the email

    # -------- Plain text (no crypto table in text mode to keep it compact) --------
