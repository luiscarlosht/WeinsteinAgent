...
# ---------------- Mailer ----------------
# Expect the local mailer:
#   from weinstein_mailer import send_email(subject, html_body, text_body, cfg_path)
# If you keep it in a sibling file, this import remains valid.
...
CRYPTO_BENCHMARK  = "BTC-USD"  # RS denominator for crypto
...
NEAR_VOL_PACE_MIN = 1.00   # <-- keep defined (fix NameError)
...
# Open Positions CSV (for the holdings table + SELL recommendations)
OPEN_POSITIONS_CSV_CANDIDATES = [
    "./output/Open_Positions.csv",
    "./output/open_positions.csv",
]

# Signals (for crypto section)
# Configure in config.yaml -> app.signals_path (csv/tsv/xlsx)
SIGNALS_PATH_DEFAULT = "./output/signals.csv"
SIGNALS_SHEET_DEFAULT = "Signals"
...
def load_config(path):
    ...
    # signals config
    signals_path = app.get("signals_path", SIGNALS_PATH_DEFAULT)
    signals_sheet = app.get("signals_sheet", SIGNALS_SHEET_DEFAULT)
    return cfg, benchmark, signals_path, signals_sheet
...
def volume_pace_today_vs_50dma(ticker, daily_df):
    """Projected full-day volume vs 50-day avg.
       Equities: 09:30–16:00 ET pacing; Crypto: UTC day pacing."""
    if isinstance(daily_df.columns, pd.MultiIndex):
        try:
...
# ---------------- Charting ----------------
def make_tiny_chart_png(ticker, benchmark, daily_df, title_suffix=""):
    ...
    fig, ax1 = plt.subplots(figsize=(4.6, 2.0), dpi=150)
    ax1.plot(close_t.index, close_t.values, label=f"{ticker}")
    ax1.plot(sma.index, sma.values, label=f"SMA{SMA_DAYS}", linewidth=1.0)
    ax1.set_ylabel("Price")
    ax1.tick_params(axis='x', labelsize=7); ax1.tick_params(axis='y', labelsize=7)
    ax2 = ax1.twinx()
    ax2.plot(rs_norm.index, rs_norm.values, linestyle="--", alpha=0.7, label="RS (norm)")
    ax2.set_ylabel("RS")
    ax2.tick_params(axis='y', labelsize=7)
    ax1.set_title(f"{ticker} — SMA{SMA_DAYS}{(' ' + title_suffix) if title_suffix else ''}", fontsize=8)
    ax1.grid(alpha=0.2)
    ...
    fig.tight_layout(pad=0.6); fig.savefig(chart_path, bbox_inches="tight"); plt.close(fig)
...
# ---------------- Signals loader (for Crypto) ----------------
def _read_csv_or_tsv(path: str) -> pd.DataFrame:
    _, ext = os.path.splitext(path.lower())
    if ext == ".tsv":
        return pd.read_csv(path, sep="\t")
    return pd.read_csv(path)

def load_signals_any(path: str, xlsx_sheet: str = "Signals") -> pd.DataFrame:
    """Attempts to read CSV/TSV/XLSX. Normalizes columns to: TimestampUTC, Ticker, Source, Direction, Timeframe."""
    ...
    df = df.rename(columns={k: v for k, v in ren.items() if k in map(str.lower, df.columns)})
    for c in ["TimestampUTC","Ticker","Source","Direction","Timeframe"]:
        if c not in df.columns: df[c] = np.nan
    # strip + uppercase direction
    df["Ticker"] = df["Ticker"].astype(str).str.strip()
    df["Direction"] = df["Direction"].astype(str).str.strip().str.upper()
    return df

# ---------------- Crypto summary (from Signals) ----------------
def _derive_stage_from_ma(price: float, ma30: float, ma_slope_week: float) -> str:
    if pd.isna(price) or pd.isna(ma30):
        return "Stage ?"
    if price >= ma30 and ma_slope_week > 0:
...
def _weekly_style_crypto_row(t, daily_df, benchmark=CRYPTO_BENCHMARK) -> Dict[str, Any]:
    """Compute weekly-like row for a crypto ticker using daily OHLC."""
    ...
    ma30_s = close.rolling(30).mean()
    ma30 = float(ma30_s.iloc[-1]) if len(close) >= 30 else np.nan

    # MA slope per week ~ 5 sessions delta
    ma_slope_week = np.nan
    if len(ma30_s.dropna()) >= 10:
        last = ma30_s.dropna().iloc[-1]
        prev = ma30_s.dropna().iloc[-5] if len(ma30_s.dropna()) >= 6 else ma30_s.dropna().iloc[0]
        ma_slope_week = float(last - prev)

    # RS vs BTC-USD
    try:
        bench_close = daily_df[("Close", benchmark)].dropna()
        aligned = close.index.intersection(bench_close.index)
        rs = (close.loc[aligned] / bench_close.loc[aligned]).dropna()
        rs_ma30 = rs.rolling(30).mean()
        rs_above_ma = bool(rs.iloc[-1] >= rs_ma30.iloc[-1]) if len(rs_ma30.dropna()) else False
        rs_slope_week = 0.0
        if len(rs_ma30.dropna()) >= 6:
            rs_slope_week = float(rs_ma30.dropna().iloc[-1] - rs_ma30.dropna().iloc[-6])
        rs_last = float(rs.iloc[-1]) if len(rs) else np.nan
        rs_ma_last = float(rs_ma30.iloc[-1]) if len(rs_ma30.dropna()) else np.nan
    except Exception:
        rs_last = np.nan; rs_ma_last = np.nan; rs_above_ma = False; rs_slope_week = np.nan

    dist_ma_pct = (price / ma30 - 1.0) * 100.0 if (pd.notna(price) and pd.notna(ma30) and ma30 != 0) else np.nan
    stage = _derive_stage_from_ma(price, ma30, ma_slope_week)

    # Buy / Watch / Avoid like weekly
    buy_signal = "Watch"
    if stage.startswith("Stage 2") and rs_above_ma and (pd.isna(dist_ma_pct) or dist_ma_pct >= 0):
        buy_signal = "Buy"
    elif stage.startswith("Stage 3") or (not rs_above_ma) or (pd.notna(dist_ma_pct) and dist_ma_pct < 0):
        buy_signal = "Avoid"
    ...
    return {
        ...
        "ma_slope_per_wk": ma_slope_week,
        "rs": rs_last,
        "rs_ma30": rs_ma_last,
        "rs_above_ma": "Yes" if rs_above_ma else "No",
        "rs_slope_per_wk": rs_slope_week
    }

def _crypto_summary_block_from_signals(signals_df: pd.DataFrame, daily_df) -> Tuple[str, str]:
    """Returns (html, text) for the Crypto Summary section, or ("","") if none."""
    if signals_df is None or signals_df.empty:
        return "", ""
    held = signals_df.copy()
    held = held[held["Direction"].str.upper().isin(["BUY","LONG","HOLD"])]
    held["Ticker"] = held["Ticker"].astype(str).str.strip()
    held = held[held["Ticker"].str.upper().str.endswith("-USD")]
    cryptos = sorted(held["Ticker"].unique().tolist())
    if not cryptos:
        return "", ""

    rows = []
    for t in cryptos:
        row = _weekly_style_crypto_row(t, daily_df, benchmark=CRYPTO_BENCHMARK)
    ...
    # counts
    buy_n = int((df["Buy Signal"] == "Buy").sum())
    watch_n = int((df["Buy Signal"] == "Watch").sum())
    avoid_n = int((df["Buy Signal"] == "Avoid").sum())
    total_n = int(len(df))

    # Tiny charts
    charts_html = ""
    for t in df["ticker"].tolist():
        _, data_uri = make_tiny_chart_png(t, CRYPTO_BENCHMARK, daily_df, title_suffix="(RS/BTC)")
        if data_uri:
            charts_html += f"""
            <div style="display:inline-block;margin:6px 8px 6px 0;vertical-align:top;text-align:center;">
              <img src="{data_uri}" alt="{t}" style="border:1px solid #eee;border-radius:6px;max-width:300px;">
              <div class="tiny">{t}</div>
            </div>
            """

    # Pretty table like weekly
    show = df[[
        "ticker","asset_class","industry","sector","Buy Signal","stage","short_term_state_wk",
        "price","ma10","ma30","dist_ma_pct","ma_slope_per_wk","rs","rs_ma30","rs_above_ma","rs_slope_per_wk"
    ]].copy()
    # formatting
    def f2(x): return "" if pd.isna(x) else f"{float(x):.2f}"
    def f2pct(x): return "" if pd.isna(x) else f"{float(x):.2f}%"
    show["price"] = show["price"].apply(f2)
    show["ma10"] = show["ma10"].apply(f2)
    show["ma30"] = show["ma30"].apply(f2)
    show["dist_ma_pct"] = show["dist_ma_pct"].apply(f2pct)
    show["ma_slope_per_wk"] = show["ma_slope_per_wk"].apply(f2)
    show["rs"] = show["rs"].apply(f2)
    show["rs_ma30"] = show["rs_ma30"].apply(f2)
    show["rs_slope_per_wk"] = show["rs_slope_per_wk"].apply(f2)

    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    header_html = f"""
    <h3>Crypto Weekly — Benchmark: {CRYPTO_BENCHMARK}</h3>
    <div class="tiny">Generated {now}</div>
    <p><b>Crypto Summary:</b> ✅ Buy: {buy_n} &nbsp;&nbsp;|&nbsp;&nbsp; 🟡 Watch: {watch_n} &nbsp;&nbsp;|&nbsp;&nbsp; 🔴 Avoid: {avoid_n} &nbsp;&nbsp; (Total: {total_n})</p>
    """

    table_html = show.rename(columns={
        "ticker":"ticker","asset_class":"asset_class","industry":"industry","sector":"sector",
        "Buy Signal":"Buy Signal","stage":"stage","short_term_state_wk":"short_term_state_wk",
        "price":"price","ma10":"ma10","ma30":"ma30","dist_ma_pct":"dist_ma_pct","ma_slope_per_wk":"ma_slope_per_wk",
        "rs":"rs","rs_ma30":"rs_ma30","rs_above_ma":"rs_above_ma","rs_slope_per_wk":"rs_slope_per_wk"
    }).to_html(index=False)

    html = header_html + charts_html + table_html
    # text fallback
    text_lines = [f"Crypto Weekly — Benchmark: {CRYPTO_BENCHMARK}",
                  f"Generated {now}",
                  f"Crypto Summary: Buy {buy_n} | Watch {watch_n} | Avoid {avoid_n} (Total {total_n})"]
    for _, r in df.iterrows():
        text_lines.append(
            f"{r['ticker']}: {r['Buy Signal']} | {r['stage']} | px {r['price']:.2f} | ma30 {r['ma30']:.2f} | dist {r['dist_ma_pct']:.2f}% | rs_above_ma {r['rs_above_ma']}"
        )
    text = "\n".join(text_lines)
    return html, text
...
def run(_config_path="./config.yaml"):
    ...
    cfg, benchmark, signals_path, signals_sheet = load_config(_config_path)
    ...
    # --------- Crypto block from Signals (placed BEFORE weekly summary) ----------
    signals_df = load_signals_any(signals_path, xlsx_sheet=signals_sheet)
    crypto_html, crypto_text = _crypto_summary_block_from_signals(signals_df, daily)
    ...
    # Compose final HTML with crypto block inserted BEFORE the weekly holdings summary
    html = html_top
    if crypto_html:
        html += "<hr/>" + crypto_html
