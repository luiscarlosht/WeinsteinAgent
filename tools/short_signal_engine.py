...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument("--window-min", type=int, default=120,
                   help="Only consider rows with elapsed_min <= this")
    p.add_argument("--bps", type=int, default=0, help="Unused (future)")
...
def summarize(df: pd.DataFrame, outdir: Path, window_min: int) -> pd.DataFrame:
    ...
    out = outdir / f"short_signal_summary_{window_min}min.csv"
...
def print_table(df_sum: pd.DataFrame):
    ...
    for _, r in df_sum.iterrows():
        print(f"{r['ticker']:<6} {r['ShortState']:<10} {int(r['NearHits']):>8} {int(r['TrigHits']):>8} {r['LastPx']:>10.2f}")
...
def compute_entry_stop_targets(entry_px: float, ma30: float, atr: float):
    ...
    atr_stop = entry + SHORT_TRAIL_ATR_MULT * atr if atr and not math.isnan(atr) else math.nan
    ma_stop = ma30 * (1 + SHORT_MA_GUARD_PCT) if ma30 and not math.isnan(ma30) else math.nan

    stop = max([c for c in [hard, atr_stop, ma_stop] if not math.isnan(c)] or [hard])
...
def track_short_exits(df: pd.DataFrame, outdir: Path):
    ...
    for t, df_t in df.groupby(ticker_col):
        ...
        ma30 = float(last.get("ma30", math.nan)) if "ma30" in df_t.columns else math.nan
        atr  = float(last.get("atr", math.nan)) if "atr" in df_t.columns else math.nan
        short_state = str(last.get("short_state", "NA"))
        trig = bool(last.get("cond_short_confirm", False))
        ...
        # OPEN SHORT position
        if not st["open"] and (short_state == "TRIGGERED" or trig):
            if not math.isnan(px):
                ...
                exit_state[t] = st
                append_exit_event(log_path, {
                    "ts": now, "ticker": t, "event": "OPEN_SHORT",
                    "price": px, "entry": entry, "stop": stop,
                    "target1": t1, "target2": t2, "atr": atr, "ma30": ma30,
                    "opened_at": now
                })
                log_ok(f"{t} SHORT opened at {entry:.2f} (T1={t1:.2f}, T2={t2:.2f}, Stop={stop:.2f})")
                continue
        ...
        if not st["hit_t1"] and px <= st["t1"]:
            st["hit_t1"] = True
            append_exit_event(log_path, {
                "ts": now, "ticker": t, "event": "READY_CLOSE_T1",
                "price": px, "entry": st["entry"], "stop": st["stop"],
                "target1": st["t1"], "target2": st["t2"],
                "atr": st["atr"], "ma30": st["ma30"],
                "opened_at": st["opened_at"]
            })
            log_ok(f"{t} SHORT ready-to-close (T1) — entry {st['entry']:.2f}, now {px:.2f}")

        # T2 hit
        if not st["hit_t2"] and px <= st["t2"]:
            st["hit_t2"] = True
            append_exit_event(log_path, {
                "ts": now, "ticker": t, "event": "READY_CLOSE_T2",
                "price": px, "entry": st["entry"], "stop": st["stop"],
                "target1": st["t1"], "target2": st["t2"],
                "atr": st["atr"], "ma30": st["ma30"],
                "opened_at": st["opened_at"]
            })
            log_ok(f"{t} SHORT ready-to-close (T2) — entry {st['entry']:.2f}, now {px:.2f}")

        # STOP hit
        if not st["stopped"] and px >= st["stop"]:
            st["stopped"] = True
            append_exit_event(log_path, {
                "ts": now, "ticker": t, "event": "STOP_HIT",
                "price": px, "entry": st["entry"], "stop": st["stop"],
                "target1": st["t1"], "target2": st["t2"],
                "atr": st["atr"], "ma30": st["ma30"],
                "opened_at": st["opened_at"]
            })
            log_warn(f"{t} SHORT STOP HIT — entry {st['entry']:.2f}, now {px:.2f}")

        # Auto-close when T2 or STOP hits
...
def main(argv=None) -> int:
    ...
    df_win = apply_window(df, args.window_min)
    if df_win.empty:
        return 0
    ...
    # NEW: track short exit events
    track_short_exits(df_win, outdir)

    if args.explain:
        ticker = args.explain.upper()
        log_info(f"Explaining {ticker} (raw view):")
