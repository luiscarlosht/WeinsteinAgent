...
class MetaDecision:
    ...
    regime_mode: str
    exposure_mode: str
    signal_quality_mode: str


def _series_value_at(series: Optional[pd.Series], dt: pd.Timestamp, default=np.nan) -> float:
    if series is None or series.empty:
        return default
    try:
        return _safe_float(series.get(dt, default), default)
    except Exception:
        return default


def _build_market_meta_cache(daily_df: pd.DataFrame) -> dict:
    """Build simple market features used by Meta Scenario F.

    This is intentionally a layer ABOVE the Weinstein CORE. It does not change
    signal generation; it only decides which portfolio profile should consume
    the available replay events on each date.
    """
    spy_close = get_panel(daily_df, "Close", "SPY")
    if spy_close.empty:
        return {}
    spy_ma30 = spy_close.rolling(30, min_periods=30).mean()
    spy_ma150 = spy_close.rolling(150, min_periods=150).mean()
    spy_ma30_slope20 = spy_ma30 - spy_ma30.shift(20)
    spy_ret20 = spy_close.pct_change(20)
    spy_high63 = spy_close.rolling(63, min_periods=20).max()
    spy_dd63 = (spy_close / spy_high63) - 1.0
    vix_close = get_panel(daily_df, "Close", "^VIX")
    return {
        "spy_close": spy_close,
        "spy_ma30": spy_ma30,
        "spy_ma150": spy_ma150,
        "spy_ma30_slope20": spy_ma30_slope20,
        "spy_ret20": spy_ret20,
        "spy_dd63": spy_dd63,
        "vix_close": vix_close,
    }
...
def _meta_decision_for_date(meta_cache: dict, dt: pd.Timestamp) -> MetaDecision:
    """Dynamic A/B/C/D/E selector for research Scenario F.

    Profiles:
      A = long-only, no PROD regime/exposure overlay
      B = both/hybrid, no PROD regime/exposure overlay
      D = PROD regime + scaled exposure
      E = D plus strict quality filter

    C is intentionally not selected by default because D is the production-like
    version of C with risk-scaled exposure. We keep C available as a standalone
    benchmark but prefer D when a regime-aware profile is selected.
    """
    px = _series_value_at(meta_cache.get("spy_close"), dt)
    ma30 = _series_value_at(meta_cache.get("spy_ma30"), dt)
    ma150 = _series_value_at(meta_cache.get("spy_ma150"), dt)
    slope20 = _series_value_at(meta_cache.get("spy_ma30_slope20"), dt)
    ret20 = _series_value_at(meta_cache.get("spy_ret20"), dt)
    dd63 = _series_value_at(meta_cache.get("spy_dd63"), dt)
    vix = _series_value_at(meta_cache.get("vix_close"), dt)

    above150 = np.isfinite(px) and np.isfinite(ma150) and px > ma150
    above30 = np.isfinite(px) and np.isfinite(ma30) and px > ma30
    rising30 = np.isfinite(slope20) and slope20 > 0
    strong_mom = np.isfinite(ret20) and ret20 > 0.04
    high_vol = np.isfinite(vix) and vix >= 25
    elevated_vol = np.isfinite(vix) and vix >= 20
    deep_dd = np.isfinite(dd63) and dd63 <= -0.10
    mild_dd = np.isfinite(dd63) and dd63 <= -0.05

    # Strong, clean bull tape: use E to prioritize high-quality alpha.
    if above150 and above30 and rising30 and strong_mom and not elevated_vol and not mild_dd:
        return MetaDecision("E", "strong_bull_quality", "both", "prod", "scaled", "strict")

    # High-volatility recovery: B historically helped in 2020-like conditions.
    if above30 and rising30 and (high_vol or deep_dd):
        return MetaDecision("B", "volatile_recovery", "both", "off", "off", "off")

    # Healthy bull: D is the production-like balanced choice.
    if above150 and rising30 and not high_vol:
        return MetaDecision("D", "healthy_bull_prod_scaled", "both", "prod", "scaled", "off")

    # Choppy or defensive tape: keep D so PROD regime/exposure can reduce risk.
    if above150 or above30:
        return MetaDecision("D", "choppy_defensive_prod_scaled", "both", "prod", "scaled", "off")

    # Bear/weak tape: B can consume short-side events, but max leverage/position
    # caps still constrain the portfolio.
    return MetaDecision("B", "weak_or_bear_hybrid", "both", "off", "off", "off")


def _passes_quality_filter(ev: pd.Series, side: str, args) -> bool:
    """Apply an E-like strict quality filter at portfolio-wrapper level.

    If replay events contain quality_score, use it. If not, fall back to
    quality_mult/reason and avoid blocking unknown legacy rows.
    """
    q = _safe_float(ev.get("quality_score"), np.nan)
    if np.isfinite(q):
        threshold = float(args.min_long_quality if side == "long" else args.min_short_quality)
        return q >= threshold
    qm = _safe_float(ev.get("quality_mult"), np.nan)
    if np.isfinite(qm):
        return qm >= 0.85
    return True
...
def run_replay_portfolio(args) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    meta_cache = _build_market_meta_cache(daily_df) if getattr(args, "meta_strategy", "off") == "F" else {}

    if args.replay_events_in:
        events = pd.read_csv(args.replay_events_in)
        log(f"Loaded replay events from → {args.replay_events_in} rows={len(events)}", level="info")
    else:
        ...
        if args.replay_events_out:
            os.makedirs(os.path.dirname(os.path.abspath(args.replay_events_out)), exist_ok=True)
            events.to_csv(args.replay_events_out, index=False)
            log(f"Wrote reusable replay events → {args.replay_events_out} rows={len(events)}", level="ok")
    ...
    meta_by_date = {}
    for d in dates:
        meta_decision = _meta_decision_for_date(meta_cache, d) if getattr(args, "meta_strategy", "off") == "F" else None
        effective_regime_mode = meta_decision.regime_mode if meta_decision else args.regime_mode
        effective_exposure_mode = meta_decision.exposure_mode if meta_decision else args.exposure_mode

        allow_long_d, allow_short_d, regime_label_d = _regime_permissions(
            daily_df, d, market_cfg, regime_mode=effective_regime_mode, neutral_policy=args.neutral_policy
        )
        long_mult_d, short_mult_d = _regime_exposure_multipliers(
            regime_label_d,
            regime_mode=effective_regime_mode,
        ...
        if short_mult_d <= 0:
            allow_short_d = False
        regime_by_date[d] = (allow_long_d, allow_short_d, regime_label_d)
        mult_by_date[d] = (float(long_mult_d), float(short_mult_d))
        if meta_decision:
            meta_by_date[d] = meta_decision
        ...
        if day_events is not None and not day_events.empty:
            ...
            if not _net_liquidation_guard(eq_now, float(args.capital), float(args.min_equity_frac)):
                ...
                equity_rows.append({
                    ...
                    "meta_profile": meta_by_date.get(dt).profile if dt in meta_by_date else "",
                    "meta_reason": meta_by_date.get(dt).reason if dt in meta_by_date else "",
                })
                continue
            ...
            for _, ev in day_events.iterrows():
                ...
                meta_decision = meta_by_date.get(dt)
                effective_mode = meta_decision.trade_mode if meta_decision else args.mode
                effective_quality_mode = meta_decision.signal_quality_mode if meta_decision else args.signal_quality_mode

                if effective_mode == "long" and sig == "SHORT":
                    continue
                if effective_mode == "short" and sig == "BUY":
                    continue

                allow_long_d, allow_short_d, _regime_label_d = regime_by_date.get(dt, (True, True, ""))
                long_mult_d, short_mult_d = mult_by_date.get(dt, (1.0, 1.0))

                t = str(ev["ticker"]).upper()
                side = "long" if sig == "BUY" else "short"
                if effective_quality_mode == "strict" and not _passes_quality_filter(ev, side, args):
                    continue
                if side == "long" and not allow_long_d:
                    continue
                if side == "short" and not allow_short_d:
                    continue
                ...
                if side == "long":
                    ...
                    size_mult = float(long_mult_d) * _safe_float(ev.get("quality_mult"), 1.0)
                else:
                    stop = core_short_stop_level(px, atr, ma30)
                    if not np.isfinite(stop) or stop <= px:
                        continue
                    per_share_risk = stop - px
                    size_mult = float(short_mult_d) * _safe_float(ev.get("quality_mult"), 1.0)
...
def main():
    ...
    ap.add_argument("--replay-events-in", default="", help="Load reusable replay events CSV and skip expensive replay generation.")
    ap.add_argument("--replay-events-out", default="", help="Write reusable replay events CSV after generation.")
    ap.add_argument("--replay-only", action="store_true", help="Only generate/load replay events and exit before portfolio accounting.")
    ...
    ap.add_argument("--meta-strategy", choices=["off", "F"], default="off", help="Dynamic meta selector above CORE. F switches between A/B/D/E using market regime features.")
    ap.add_argument("--meta-log", action="store_true", help="Write daily meta selector decisions into the equity CSV.")
    ...
    if args.replay_only:
        if args.save_events and not args.replay_events_out:
            events.to_csv(events_path, index=False)
            log(f"Wrote replay events → {events_path}", level="ok")
        log(f"Replay-only complete. Events={len(events)}", level="ok")
        return
    ...
    summary = pd.DataFrame([{
        ...
        "meta_strategy": getattr(args, "meta_strategy", "off"),
    }])
